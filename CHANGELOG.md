...
### Added
- **DeepSeek Harness (`dsh`) support** Ã¢â‚¬â€ `npx graphyloop install` now wires
  ...
  the 15 tools appear without touching a bundle and without a pnpm step Ã¢â‚¬â€ dsh
  ...
  `mcp__graphyloop__<name>` (`mcp__graphyloop__plan_feature`, Ã¢â‚¬Â¦).
...
- **`graphyloop-squad` skill (dsh only).** dsh has no agent files and no
  file-based slash commands Ã¢â‚¬â€ agents are cordis compositions and commands are
  plugins Ã¢â‚¬â€ so a squad of 25 `.md` agents and 15 `/chadi-*` commands has nowhere to
...
- `skills_status` (and the engine's `skills` command) now also read the dsh skill
  roots Ã¢â‚¬â€ `<project>/.dsh/skills`, `<project>/.agents/skills`, `$DSH_HOME/skills`,
  `~/.agents/skills` Ã¢â‚¬â€ so a skill installed for dsh is reported as present instead
  of missing.
...
- **Full skill library embedded (65 skills).** `skills/` now ships the complete
  ...
  scans `skills/`); the never-overwrite rule per skill name is unchanged.
- Update notes: re-sync from the live global set with
  `robocopy %USERPROFILE%\.config\opencode\skills .\skills /MIR /XD scripts\vendor`.

## [0.2.1] Ã¢â‚¬â€ 2026-08-16
...
- **`npm run dev` hung a Windows agent session forever.** The shell tool reads a
  ...
  pipe's write end is closed Ã¢â‚¬â€ not when the direct child exits. Detaching a dev
  ...
  2.3 s while the caller's stdout EOF only arrived at 21.8 s Ã¢â‚¬â€ the moment the
  ...
  `bInheritHandles=FALSE`. **EOF lag 19,498 ms Ã¢â€ â€™ 8 ms**, with the server still
  answering `200` after the call returns.
- **The workflow rules recommended the broken pattern.** `workflow/AGENTS.md`
  called `Start-Process` with both redirects "the ONLY safe detach on Windows Ã¢â‚¬Â¦
...
- **`server-guard` plugin and `start-server.ps1` now ship with the kit.** Both
  were documented in the workflow rules but installed by nothing Ã¢â‚¬â€ the rules told
  agents to use a launcher that did not exist, flagged in-line as "[OPTIONAL Ã¢â‚¬â€
  ...
  of stalling the turn. Windows-only by design Ã¢â‚¬â€ the launcher is PowerShell and
...
- The installer now copies **every** `plugin/<name>/` directory rather than only
  ...
  edit. It never creates that legacy path Ã¢â‚¬â€ the plugin-local copy is canonical.
...
- **Bundled skills.** Every agent routed on skills by name while graphyloop
  ...
  back Ã¢â‚¬â€ a new install was weaker than the workflow it advertised. Five
  ...
  of the same name is **never** overwritten Ã¢â‚¬â€ not by `--force`, not by `update`;
...
- **`chadi-integrator`** Ã¢â‚¬â€ the wave-2 join had no owner: the planner handed it to
...
- **Wave planner Ã¢â‚¬â€ `plan_feature` (MCP) / `graphyloop_plan_feature` (plugin) /
  `plan --goal` (CLI).** "I want an inventory system" now decomposes into
  contract (one agent freezes schema + routes + props + test scenarios) Ã¢â€ â€™
  **database Ã¢Ë†Â¥ backend Ã¢Ë†Â¥ frontend Ã¢Ë†Â¥ tests** Ã¢â€ â€™ integration Ã¢â€ â€™ **test Ã¢Ë†Â¥ typecheck Ã¢Ë†Â¥
  security Ã¢Ë†Â¥ performance Ã¢Ë†Â¥ review** Ã¢â€ â€™ gated deploy. Every task carries its
...
- **Supabase + Vercel credential layer.** `secrets_status` (masked readiness
  report Ã¢â‚¬â€ which keys exist, where each comes from, what is missing; never
  ...
  destructive step carries its approval gate Ã¢â‚¬â€ it executes nothing).
...
- `stack` CLI command (framework, package manager, database layer, deploy
  target, migrations, scripts) Ã¢â‚¬â€ evidence-based detection used by the planner and
  preflight.
...
- **A failing assertion in the installer suite hung the entire test run.** The
  ...
  skipped `stdin.end()` Ã¢â‚¬â€ the spawned server kept its stdio pipes open and
...
- **MCP tools were inert until a manual init.** The contracted tool set has no
  ...
  a fresh project answered every call with `{"error":"not initialized"}` Ã¢â‚¬â€
...
- **Parallel agents silently dropped each other's writes.** Every command is
  load Ã¢â€ â€™ mutate Ã¢â€ â€™ save, so concurrent writers raced. Measured on an 1800-entry
  store with 12 concurrent writes: 6 of 12 lost. State is now guarded by an
  atomic `mkdir` lock with a stale-lock break Ã¢â‚¬â€ same run after the fix: 12 of 12.
...
## [0.1.1] Ã¢â‚¬â€ 2026-08-14

- Complete rebrand to the GraphyLoop identity (engine, agents, tool names,
  config entries).
- `graphcrew` agent squad.
- Automatic npm releases via GitHub Actions (tag Ã¢â€ â€™ test Ã¢â€ â€™ publish).
- Copy-paste setup prompt for any AI harness.
- CI matrix: Windows/macOS/Linux Ãƒâ€” Node 20/22/24.

## [0.1.0]

- Initial release Ã¢â‚¬â€ one-command install for OpenCode, Claude Code, Codex and
  Cursor Ã‚Â· 24-agent squad Ã‚Â· 5-gate workflow Ã‚Â· MCP server Ã‚Â· persistent memory and
  swarm engine Ã‚Â· zero runtime dependencies.
