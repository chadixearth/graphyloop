...
| 📚 **11 bundled skills** | Contract-first waves, API hardening, frontend security, accessibility, web performance, dependency audit, Supabase, Vercel, secrets, memory — graphyloop-authored, wired into the agents that own them. A skill you already have is never overwritten |
...
**Skills** — 11 graphyloop-authored skills install with the squad, so a fresh setup has the discipline its agents already route on:
...
Agents also route on skills from other collections ([superpowers](https://github.com/obra/superpowers): `brainstorming`, `systematic-debugging`, `tdd-workflow`, `writing-plans`, `verification-before-completion`, plus `security-review`, `error-handling`, `postgres-patterns`, `deployment-patterns`, design skills, `council`, `last30days`). Those are not bundled — install what you use. Call `skills_status` to see exactly which skills are present on a machine and which the squad expects; agents state a missing skill in one line rather than faking it, and a test blocks any agent that starts routing on a skill nobody tracks.
...
```bash
npm test              # 160 tests across 10 suites
