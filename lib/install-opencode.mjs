...
import { backupFile, matchesFile, timestamp } from './fsutil.mjs';
...
function stepAgents(configDir, force, log) {
  ...
  for (const entry of entries) {
    ...
    if (existsSync(destFile)) {
      ...
      if (matchesFile(destFile, srcFile)) {
        skipped++;
...
function stepConfigMerge(configDir, log, warnings) {
  ...
  if (merged > 0) {
    if (existed) {
      const backupPath = `${configFile}.bak-${timestamp()}`;
      copyFileSync(configFile, backupPath);
      log(`    backup ${path.basename(configFile)} -> ${path.basename(backupPath)}`);
    }
