...
function stepAgents(configDir, force, log) {
  ...
  for (const entry of entries) {
    ...
    if (existsSync(destFile)) {
      ...
      log(`    backup ${entry} -> ${entry}.bak-${timestamp()}`);
    }
...
/** Install plugin/graphyloop/* -> configDir/plugins/graphyloop (force-gated, backed up). */
function stepPlugin(configDir, force, log) {
  log('[opencode/0] Installing graphyloop plugin (OpenCode graphyloop_* tools)');
