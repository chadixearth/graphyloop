 ...
 */
function copyDir(src, dest, force, log) {
  ...
  for (const entry of readdirSync(src)) {
    ...
    if (isDirectory(srcPath)) {
      const sub = copyDir(srcPath, destPath, force, log);
      copied += sub.copied;
      skipped += sub.skipped;
    } else {
...
export async function install(ctx = {}) {
  ...
  let r = copyDir(adapterSrc, graphyloopDir, force, log);
  if (r.missing) {
  ...
  log('[core/3] Installing MCP server (mcp-server.mjs)');
