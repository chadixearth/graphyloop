...
//   3. skills/            — the bundled skills plus the dsh-only
 ...
 *
 * The `env` block is load-bearing, not decoration: dsh is a long-lived host whose
 * cwd is wherever it was launched (usually the home directory), while the project
 * is the workspace picked in the UI. The MCP server therefore cannot read the
 * project from its cwd — GRAPHYLOOP_DSH_HOME tells it which dsh home to read the
 * workspace store from. `dsh-mcp-client` merges this env over a scrubbed parent
 * env that strips every `DSH_*` name, so an explicit entry is the only way in.
 *
 * @param {string} mcpPath absolute path of ~/.graphyloop/mcp-server.mjs
 * @param {string} dshDir  absolute dsh home ($DSH_HOME or ~/.dsh)
 * @returns {string}
 */
export function patchBlock(mcpPath, dshDir) {
  return [
    ...
    '        env:',
    '          # dsh keeps the open project in its workspace store, not in the',
    '          # host process cwd — this is where the server reads it from.',
    '          # Add GRAPHYLOOP_PROJECT_ROOT here to pin one project instead.',
    `          GRAPHYLOOP_DSH_HOME: ${yamlSingleQuoted(dshDir)}`,
    '',
  ].join('\n');
}

/**
 * Blocks earlier versions wrote, newest first.
 *
 * An install/update replaces one of these with the current block (it is
 * graphyloop-owned), and uninstall still recognises it as ours. A row that
 * matches none of them was edited by the user and is left alone.
 *
 * @param {string} mcpPath absolute path of ~/.graphyloop/mcp-server.mjs
 * @returns {string[]}
 */
export function legacyPatchBlocks(mcpPath) {
  return [
    // 0.3.0: no env block, so the server fell back to the host cwd and every
    // tool call failed the project-root guard.
    [
      '# graphyloop MCP server — managed by `npx graphyloop install`; removed by',
      '# `npx graphyloop uninstall`. Tools appear as mcp__graphyloop__<name>.',
      '- insert:',
...
export async function install(ctx) {
  ...
  if (noConfigMerge) {
    ...
    const block = patchBlock(mcpPath, dshDir);
    if (content.includes(block)) {
      log(`    skip  cordis.patch.yml (id: ${PATCH_ROW_ID} already current)`);
      report.skipped++;
    } else if (hasPatchRow(content)) {
      // The row exists but is not the current block. When it is one graphyloop
      // wrote itself, refresh it — an install from before the env block left
      // every tool call failing the project-root guard, and that is a bug fix
      // the user cannot be expected to hand-apply. An edited row is theirs.
      const stale = legacyPatchBlocks(mcpPath).find((b) => content.includes(b));
      if (stale) {
        await backupFileAsync(patchFile, log);
        await fs.writeFile(patchFile, content.replace(stale, block), 'utf8');
        log(`    upgraded id: ${PATCH_ROW_ID} in ${patchFile} (added env: GRAPHYLOOP_DSH_HOME)`);
        report.merged++;
      } else {
        log(`    skip  cordis.patch.yml (id: ${PATCH_ROW_ID} present but user-modified)`);
        report.skipped++;
        report.warnings.push(
          `${patchFile}: the ${PATCH_ROW_ID} row was edited, so it was left alone — add ` +
          `"env:" with "GRAPHYLOOP_DSH_HOME: ${dshDir}" under its config, or the server cannot ` +
          'find the project you have open in dsh.'
        );
      }
    } else {
      await backupFileAsync(patchFile, log);
    ...
    await fs.writeFile(patchFile, `${patchFileHeader()}${patchBlock(mcpPath, dshDir)}`, 'utf8');
...
export default { install, patchBlock, legacyPatchBlocks, patchFileHeader, hasPatchRow, isEmptyPatchList, PATCH_ROW_ID };
