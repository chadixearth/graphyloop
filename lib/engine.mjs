...
/**
 * Lowercased searchable text per memory entry, built on first use.
 *
 * Keyed by the entry object in a WeakMap so nothing has to invalidate it: the
 * entry is immutable once stored, and a state re-parse yields fresh objects that
 * simply get their own text. Shared across engines in the process because the
 * key is the object itself, not a path.
 */
const SEARCH_TEXT = new WeakMap();
function searchText(m) {
  let text = SEARCH_TEXT.get(m);
  if (text === undefined) {
    text = (m.metadata ? `${m.content} ${JSON.stringify(m.metadata)}` : String(m.content ?? '')).toLowerCase();
    SEARCH_TEXT.set(m, text);
  }
  return text;
}
...
// Allocating a SharedArrayBuffer per wait showed up under lock contention, and
// the wait is the one place we are deliberately doing nothing — so it is hoisted.
const SLEEP_VIEW = new Int32Array(new SharedArrayBuffer(4));
function sleepSync(ms) {
  Atomics.wait(SLEEP_VIEW, 0, 0, ms);
}

// Escalating lock backoff. A state write is ~1 ms, so the first retries are
// sub-millisecond; 20 ms is reserved for a holder that is genuinely slow.
const LOCK_BACKOFF_MS = [0.25, 0.5, 1, 2, 4, 8, 16, 20];
...
export function taskReadiness(queue) {
  ...
  // The common case for a fresh or memory-only session is an empty queue, and
  // status() calls this on every single request.
  if (tasks.length === 0) return { ready: [], blocked: [], byWave: {} };
...
export function createEngine(opts = {}) {
  ...
  const prettyState = opts.prettyState ?? process.env.GRAPHYLOOP_PRETTY_STATE === '1';
  ...
  // Hot-path caching
  //
  // Every tool call used to pay existsSync + readFileSync + JSON.parse of the
  // whole state file — 2.4 ms for a read-only `status` at 800 memories, and the
  // same cost again inside every write. A harness makes hundreds of calls per
  // session, so that parse was the dominant latency in the whole server.
  //
  // The cache is validated against the file's stat signature, never against a
  // timer: mtime + size + inode + ctime. Our own writes are tmp-file + rename,
  // which changes the inode every time, so any write by any process — this
  // engine, a spawned CLI, a second harness — invalidates the cache on the next
  // call. A stale read is therefore not possible, only a re-parse.
  // ---------------------------------------------------------------------------
  let cache = null;        // { sig, state }
  let dirEnsured = false;  // .graphyloop/ exists (checked once, not per write)
  let migrationTried = false;
  let savedInTxn = false;  // did the current withLock body actually persist?
  const metrics = { loads: 0, parses: 0, writes: 0, lockWaits: 0, searches: 0 };

  function statSig(st) {
    // ino is 0 on some Windows filesystems, so never rely on it alone.
  ...
  function loadState() {
    ...
    try {
      ...
      // No state file. The legacy location only matters in that case, so the
      // migration probe costs two syscalls once per engine instead of per call.
      if (!migrationTried) {
        migrationTried = true;
        if (migrateLegacyState()) return loadState();
      }
      cache = null;
      return freshState();
    }
    const sig = statSig(st);
    if (cache && cache.sig === sig) return cache.state;
    metrics.parses++;
    let state;
    try {
  ...
  function saveState(s) {
    ...
    //
    // Compact by default: two-space indent nearly doubled the bytes written on
    // every single mutation (a 2000-memory store is ~1 MB pretty vs ~600 KB
    // compact) and the file is machine state, not a config a human edits. Set
    // GRAPHYLOOP_PRETTY_STATE=1 when you do want to read it by eye.
    const tmp = `${statePath}.tmp-${process.pid}`;
    writeFileSync(tmp, prettyState ? JSON.stringify(s, null, 2) : JSON.stringify(s));
    renameSync(tmp, statePath);
    metrics.writes++;
    savedInTxn = true;
    // Re-stamp rather than drop: the next call (very often a read of what was
    // just written) then costs one stat instead of a full parse.
    try {
      cache = { sig: statSig(statSync(statePath)), state: s };
    } catch {
      cache = null;
    }
  }
  ...
  //
  // Uncontended cost is one syscall: the parent directory is created on demand
  // from the ENOENT path instead of an unconditional recursive mkdir per write.
  function acquireLock() {
    ...
    for (;;) {
      try {
        ...
        if (e.code === 'ENOENT' && !dirEnsured) {
          ensureStateDir(); // first write in a fresh project
          continue;
        }
        ...
        metrics.lockWaits++;
        // A state write takes ~1 ms, so the old flat 20 ms sleep made every
        // contended writer wait an order of magnitude longer than the work it
        // was waiting for. Escalate instead: sub-ms first, 20 ms only for a
        // genuinely long holder.
        sleepSync(LOCK_BACKOFF_MS[Math.min(attempt++, LOCK_BACKOFF_MS.length - 1)]);
      }
    }
  }
  ...
  function withLock(fn) {
    ...
    try {
      ...
      // A body that mutated the loaded state but returned before saving (an
      // error path) must not leave that mutation visible in the cache. Dropping
      // it costs one re-parse on the next call and keeps the cache honest.
      if (!savedInTxn) cache = null;
      releaseLock(lock);
    }
  }
  ...
  //
  // detectStack() reads package.json, lockfiles, config files and the schema
  // tree. plan_feature, env_sync and preflight each called it fresh, so planning
  // one feature re-scanned the project several times. The manifest's stat
  // signature is the invalidator: install a dependency or edit package.json and
  // the next call re-detects.
  // ---------------------------------------------------------------------------
  let stackCache = null; // { sig, stack }

  function manifestSig() {
    try {
      const st = statSync(resolve(projectRoot, 'package.json'));
  ...
  function cachedStack() {
    const sig = manifestSig();
    if (stackCache && stackCache.sig === sig) return stackCache.stack;
    let stack = null;
    let error = null;
    try {
  ...
  return {
     ...
     *
     * The searchable text of an entry is built once and kept in a WeakMap keyed
     * by the entry itself: recall is the most-called tool in the workflow, and
     * re-running JSON.stringify + toLowerCase over the entire store on every
     * query was the whole cost of a search. Entries are immutable once stored,
     * and a re-parsed state produces new objects, so the cache cannot go stale;
     * it simply disappears with the state it described.
     */
    memorySearch({ query, limit, type } = {}) {
      ...
      const multiTerm = termCount > 1;
      const now = Date.now();
      const all = Array.isArray(s.memories) ? s.memories : [];
      ...
      for (const m of all) {
        ...
        if (hits === 0) continue;
        let score = hits / termCount;
        // A single-term query already matched the whole phrase.
        if (!multiTerm || text.includes(q)) score += 3 / termCount;
        ...
        scored.push({ ...m, score: Math.round(score * 1000) / 1000 });
      }

      const results = scored.sort((a, b) => b.score - a.score || b.timestamp - a.timestamp).slice(0, k);
      return { ok: true, total: all.length, searched, results };
    },
    ...
    envSync({ target, providers, framework, force } = {}) {
      ...
      if (!fw) {
        const stack = cachedStack();
        fw = stack ? stack.framework : undefined;
      }
      return secrets.envSync({ target, providers, framework: fw, force });
    },

    preflight({ target } = {}) {
      return runPreflight({ projectRoot, target, stack: cachedStack() });
    },

    stack() {
      const detected = cachedStack();
      if (!detected) return { error: `stack detection failed: ${stackCache?.error || 'unknown reason'}` };
      return { ok: true, ...detected };
    },

    /**
     * Diagnostics for the hot path — how many times state was loaded, how many
     * of those actually re-parsed the file, writes, lock waits, searches.
     *
     * This exists so "the cache works" is a test assertion rather than a claim:
     * N read-only calls must cost exactly one parse. Also handy when a harness
     * feels slow — a parse count equal to the load count means something outside
     * this process is writing state between every call.
     */
    metrics() {
      return { ...metrics, cached: Boolean(cache), stateFile: statePath };
    },
