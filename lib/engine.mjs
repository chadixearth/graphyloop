...
// Skills graphyloop ships and installs itself (lib/install-skills.mjs). Listed
// here too, by name only, because the engine runs from ~/.graphyloop where the
// repo's skills/ directory does not exist.
export const GRAPHYLOOP_SKILLS = [
  'graphyloop-waves',
  'secrets-hygiene',
  'supabase-setup',
  'swarm-memory',
...
// Skills the squad's agent files route on but does not ship — they come from the
// user's own collections. Reported as missing so an agent can say so in one line
// instead of pretending to have loaded one.
