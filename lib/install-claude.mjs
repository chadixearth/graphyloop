...
function backupFile(file, log) {
  ...
  log(`    backup ${path.basename(file)} -> ${path.basename(backup)}`);
}
...
function installAgents(claudeRoot, force, warnings, log) {
  ...
  for (const entry of entries) {
    ...
    writeFileSync(destFile, claudeAgentFromSource(sourceText, entry));
    copied++;
    log(`    copy  ${entry}`);
  }
...
function installCommands(claudeRoot, force, warnings, log) {
  ...
  for (const name of names) {
    ...
    writeFileSync(
      destFile,
      claudeCommandFile(name, cmd.description, cmd.template)
    );
    copied++;
    log(`    copy  ${name}.md`);
  }
