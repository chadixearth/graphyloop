 ...
 * @param {object} [args.stack] already-detected stack — the engine caches it, so
 *   passing it here avoids a second full project scan per preflight call
 */
export function preflight({ projectRoot, target = 'all', env, stack: detected } = {}) {
  ...
  const stack = detected || detectStack(projectRoot);
