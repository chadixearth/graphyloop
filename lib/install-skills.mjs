...
// the one it advertises. The five core skills are graphyloop-authored; since
// 0.3.0 the full personal library (65 skills: design systems, branding, video,
// research, workflow, domain packs) ships as a snapshot from
// chadixearth/opencode-skills, so every agent skill reference resolves on a fresh
// install. Third-party collections (superpowers, GSAP, three.js) are included as
// the user's curated copies.
//
...
export function installSkills(ctx = {}) {
  ...
  for (const name of names) {
    ...
    const copied = copyTree(path.join(src, name), dest);
...
export default { installSkills, bundledSkills, skillFiles, DSH_SKILLS_SRC };
