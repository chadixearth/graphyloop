 ...
 * The block is removed only when it is still byte-identical to what the
 * installer wrote — an edited block is theirs now. A patch file that ends up
 * with no entries keeps a `[]` body, because dsh throws at boot on a patch file
 * that does not parse as an array.
 */
function removeDshPatchRow(patchFile, mcpPath, log, removed) {
  ...
  const block = patchBlock(mcpPath);
