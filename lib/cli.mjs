...
//   --harness opencode|claude|codex|cursor|dsh|all (default: all detected; "all" forces every one)
...
import { bundledSkills, skillFiles, DSH_SKILLS_SRC } from './install-skills.mjs';
...
import {
  ...
  hasPatchRow,
  isEmptyPatchList,
  PATCH_ROW_ID as DSH_PATCH_ROW_ID,
  LIBRARY_DIR as DSH_LIBRARY_DIR,
...
const HARNESS_NAMES = ['opencode', 'claude', 'codex', 'cursor', 'dsh'];
...
Usage:
  graphyloop install [--harness opencode|claude|codex|cursor|dsh|all] [flags]
...
Flags:
  --harness <name>       opencode | claude | codex | cursor | dsh | all (default all)
...
function selectHarnesses(opts, detected) {
  ...
  // --harness all: install for every supported harness regardless of detection
...
async function cmdInstall(opts, stdout, stderr) {
  ...
  if (names.length === 0) {
    // Fresh machine with no harness configs yet: install all of them so the
    // first run "just works" no matter which harness the user opens next.
    stdout.write(`No harnesses detected under ${opts.home} — installing for all ${HARNESS_NAMES.length} harnesses (${HARNESS_NAMES.join(', ')}) so you are ready either way. Pass --harness <name> to target one.\n`);
 ...
 */
function uninstallSkills(skillsRoot, log, removed, srcDir) {
  if (!existsSync(skillsRoot)) return;
  for (const [rel, content] of skillFiles(srcDir)) {
    const full = path.join(skillsRoot, rel);
    removeIfMatches(full, content, log, removed);
  }
  for (const name of bundledSkills(srcDir)) {
...
/**
 * Remove the graphyloop `insert` block from a dsh patch layer.
 *
 * Text surgery on purpose: the file is the user's own layer, and re-emitting it
 * through a YAML serializer would drop their comments and `!!js` expressions.
 * The block is removed only when it is still byte-identical to a block the
 * installer wrote (current or any earlier shape) — an edited block is theirs
 * now. A patch file that ends up with no entries keeps a `[]` body, because dsh
 * throws at boot on a patch file that does not parse as an array.
 */
function removeDshPatchRow(patchFile, mcpPath, dshDir, log, removed) {
  if (!existsSync(patchFile)) {
    log(`    skip ${patchFile}: not present (nothing graphyloop added)`);
    return;
  }
  let content;
  try { content = readFileSync(patchFile, 'utf8'); } catch (err) {
    log(`    WARN: cannot read ${patchFile} (${err.message}); leaving it untouched`);
    return;
  }
  const block = [patchBlock(mcpPath, dshDir), ...legacyPatchBlocks(mcpPath)]
    .find((candidate) => content.includes(candidate));
  if (!block) {
    log(
      hasPatchRow(content)
        ? `    skip ${patchFile}: the ${DSH_PATCH_ROW_ID} row differs from the graphyloop copy (user-modified)`
        : `    skip ${patchFile}: no ${DSH_PATCH_ROW_ID} row (nothing graphyloop added)`
    );
    return;
  }
  backupFile(patchFile, log);
  const rest = content.replace(block, '');
  if (isEmptyPatchList(rest)) {
    const header = patchFileHeader();
    if (content === `${header}${block}`) {
      // graphyloop created this file and nothing else was added to it.
      try {
        unlinkSync(patchFile);
        removed.push(patchFile);
        log(`    remove ${patchFile}`);
      } catch (err) {
        log(`    WARN: could not remove ${patchFile}: ${err.message}`);
      }
      return;
    }
    // Keep the user's comments, restore the empty list dsh can parse.
    const kept = rest.replace(/^\s*\[\s*\]\s*$/m, '').replace(/\n{3,}/g, '\n\n').trimEnd();
    writeFileSync(patchFile, `${kept}\n[]\n`);
  } else {
    writeFileSync(patchFile, `${rest.replace(/\n{3,}/g, '\n\n').trimEnd()}\n`);
  }
  removed.push(`id: ${DSH_PATCH_ROW_ID} in ${patchFile}`);
  log(`    removed the ${DSH_PATCH_ROW_ID} row from ${patchFile}`);
}

function uninstallDsh(home, opts, log, removed) {
  const dshDir = dshHome(home);
  log(`  [dsh] ${dshDir}`);

  removeDshPatchRow(
    path.join(dshDir, 'cordis.patch.yml'),
    path.join(home, '.graphyloop', 'mcp-server.mjs'),
    dshDir,
    log,
    removed
  );
  ...
  const wf = path.join(REPO_ROOT, 'workflow', 'AGENTS.md');
  if (existsSync(wf)) removeIfMatches(path.join(dshDir, 'AGENTS.md'), readFileSync(wf, 'utf8'), log, removed);

  const skillsRoot = path.join(dshDir, 'skills');
  uninstallSkills(skillsRoot, log, removed);
  uninstallSkills(skillsRoot, log, removed, DSH_SKILLS_SRC);

  // The squad prompt library: only files still byte-identical to the shipped copy.
  const libRoot = path.join(dshDir, DSH_LIBRARY_DIR);
  removeMatching(path.join(libRoot, 'agents'), contentMap(path.join(REPO_ROOT, 'agents'), '.md'), log, removed);
  removeMatching(
    path.join(libRoot, 'commands'),
    contentMap(path.join(REPO_ROOT, 'templates', 'codex', 'prompts'), '.md'),
    log,
    removed
  );
  for (const dir of [path.join(libRoot, 'agents'), path.join(libRoot, 'commands'), libRoot]) {
    try {
      if (existsSync(dir) && readdirSync(dir).length === 0) {
        rmdirSync(dir);
        log(`    remove dir ${dir}`);
      }
    } catch { /* keep it: user files are still in there */ }
  }
}
