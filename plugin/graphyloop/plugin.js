  ...
  return {
    ...
    tool: {
      ...
      graphyloop_skills: tool({
        description: 'Which skills are installed on this machine (project .opencode/skills, .dsh/skills, .agents/skills; ~/.config/opencode/skills, ~/.claude/skills, ~/.dsh/skills, ~/.agents/skills), which graphyloop-bundled skills are present (graphyloop-waves, api-contract-design, api-hardening, frontend-security, web-accessibility, web-performance, dependency-audit, supabase-setup, vercel-deploy, secrets-hygiene, swarm-memory), and which skills the squad routes on but are missing. Check here instead of guessing — a missing skill is reported in one line, never faked.',
        args: {},
        async execute() {
