...
# using `opencode --print` to capture real end-to-end output.
...
run_version() {
  ...
  for i in "${!QUERIES[@]}"; do
    ...
    # Run opencode --print with the skill invocation
    # No timeout — opencode --print exits on its own; kill manually if stuck
    if "$CLAUDE" --print \
      "/last30days $query" \
